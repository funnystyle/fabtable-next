		...
		<>
			...
			<Form form={form} layout="vertical" className="modal-input-area">
				{Array.from({ length: searchCount }, (_, i) => (
					<SearchModalNumberForm key={i} form={form} order={order} searchLocation={searchLocation} searchDiv={searchDiv} index={i + 1} searchCount={searchCount} setSearchCount={setSearchCount} data={data} setData={setData} />
				))}
			</Form>
		</>
	);
